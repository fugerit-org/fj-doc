...
<#macro handleMultiElement multiElement separator>
	<#if multiElement.xsdElements??><phrase>(</phrase><#list multiElement.xsdElements as element><#if element?index != 0><phrase>${separator}</phrase></#if><phrase <#if (element.type)?? >link="#type_${element.type}"</#if>>${element.rawName}</phrase></#list><phrase>)<@handleMultiElementOccurs multiElement=multiElement/></phrase></#if>
